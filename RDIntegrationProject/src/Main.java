...
/**
 * A basic program containing examples of the concepts learned in COP 2006
 * 
...
public class Main {
  ...
  /**
   * Contains code for integration project requirements not needed for main program functionality
   */
  public static void moreExamples() {
    ...
    for (int i = 0; i < numArray.length; i++) {
      for (int j = 0; j < numArray[i].length; j++) {
        if (numArray[i][j] == searchNum) {
          System.out.println(searchNum + " found at position: " + "(" + i + ", " + j + ")");
        }
      }
    }
    ...
    System.out.print("Enter the number of cats to fill the array: ");
    int numCats = scan.nextInt();
    
    Animal[] cats = new Animal[numCats];
    
    //cats array will be filled with cat objects even though it is
    //of type animal. This works because Cat is a subclass of 
    //Animal, demonstrating polymorphism
    for (int i = 0; i < cats.length; i++) {
      cats[i] = new Cat();
    }
    
    System.out.println("The array has been filled with " + numCats + " cats");
  }
